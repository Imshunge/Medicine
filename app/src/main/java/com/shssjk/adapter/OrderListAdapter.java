...
import com.shssjk.model.OrderButtonModel;
...
public class OrderListAdapter extends BaseAdapter implements View.OnClickListener {
	...
	public void onClick(View v) {
		...
		if (mHandler!=null){
			Message msg = mHandler.obtainMessage(MobileConstants.MSG_CODE_ORDER_BUTTON_ACTION);
			SPOrder order = mOrders.get(position);
			msg.obj = order;
			msg.what = tag;
	...
	private void buildProductButtonGallery(LinearLayout gallery , List<OrderButtonModel> buttonModels , int position){
		...
		if (buttonModels !=null && buttonModels.size() > 0){
			...
			button.setText("再次购买");
			button.setFocusable(false);
			button.setTag(R.id.key_tag_index1, MobileConstants.tagBuyAgain);
			button.setTag(R.id.key_tag_index2, position);
			button.setBackgroundResource(R.drawable.tag_button_bg_unchecked);
			button.setOnClickListener(this);
			gallery.addView(view);
	...
	public List<OrderButtonModel> getOrderButtonModelByOrder(SPOrder order) {
		...
		if (order.getPayBtn() == 1) {//显示支付按钮
			OrderButtonModel payModel = new OrderButtonModel("支付", MobileConstants.tagPay, true);
			buttons.add(payModel);
		}

		if (order.getCancelBtn() == 1) {//取消订单按钮
			OrderButtonModel cancelModel = new OrderButtonModel("取消订单", MobileConstants.tagCancel, false);
			buttons.add(cancelModel);
		}

		if (order.getReceiveBtn() == 1) {//确认收货按钮
			OrderButtonModel cancelModel = new OrderButtonModel("确认收货", MobileConstants.tagReceive, true);
			buttons.add(cancelModel);
		}

		if (order.getShippingBtn() == 1) {//查看物流按钮
			OrderButtonModel cancelModel = new OrderButtonModel("查看物流", MobileConstants.tagShopping, true);
			buttons.add(cancelModel);
		}
		return buttons;
	}
	...
	public View.OnClickListener orderButtonClickListener = new View.OnClickListener(){
		...
		public void onClick(View v) {
			...
			if (mHandler!=null){
				Message msg = mHandler.obtainMessage(MobileConstants.MSG_CODE_ORDER_LIST_ITEM_ACTION);
				SPOrder order = mOrders.get(position);
				msg.obj = order;
				mHandler.sendMessage(msg);
			}
