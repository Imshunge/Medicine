...
public class SPSearchkeyListAdapter extends BaseAdapter {
		...
		switch (itemViewType){
			case TYPE_HOT_KEY:
				if (convertView == null){
					...
					holder.buttonGallery = (GridLayout)convertView.findViewById(R.id.search_hotkey_lyaout);
	...
	private void buildProductButtonGallery(GridLayout gallery){
		gallery.removeAllViews();
		List<String> hotKeys = SPServerUtils.getHotKeyword();
		if (hotKeys !=null && hotKeys.size() > 0){
