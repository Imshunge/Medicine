...
package com.shssjk.http.community;
...
public class CommunityRequest {
	...
	private static String TAG = "CommunityRequest";
	 ...
	 * @param ofseet
	 * @param keyword
	 * @param r  offset（分页起始变量）、r（分页页数）、keyword（模糊查询字段
	 * @param category_id
	 * @param successListener
	 * @param failuredListener
	 */
	public static void  getSchoolcCategory(String ofseet,String keyword,int r ,String  category_id,final SPSuccessListener successListener, final SPFailuredListener failuredListener) {
...
//		}
		...
		SPMobileHttptRequest.get(url, params, new JsonHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
				...
				try {
					...
					int status = response.getInt(Response.STATUS);
					List<ComCategory> filters = new ArrayList<ComCategory>();
					if (status > 0) {
						/** 工具类json转为User实体 **/
						JSONArray resultJson = (JSONArray) response.getJSONArray(Response.DATA);
						filters = SPJsonUtil.fromJsonArrayToList(resultJson, ComCategory.class);
						successListener.onRespone(msg, filters);
					} else {
						failuredListener.onRespone(msg, -1);
					}
	 ...
	 *
	 */
	public static void  getMySchoolList(String ofseet,int r,final SPSuccessListener successListener, final SPFailuredListener failuredListener) {

		assert(successListener!=null);
		assert(failuredListener!=null);
		String url =  SPMobileHttptRequest.getRequestUrl("Quack", "my_quack");
...
//		}
	 ...
	 *
	 */
	public static void  getMySchoolArticlList(String ofseet,String keyword,int r ,String  cid,final SPSuccessListener successListener, final SPFailuredListener failuredListener) {

		assert(successListener!=null);
		assert(failuredListener!=null);
		String url =  SPMobileHttptRequest.getRequestUrl("Quack", "my_article");
		RequestParams params = new RequestParams();
		if(!SSUtils.isEmpty(ofseet)) {
			params.put("offset", ofseet);
		}
...
//		Information/comment_list
	 ...
	 * @param failuredListener
	 */
	public static void getComCheckUserListWithCID(String cid, String offset, String r, final SPSuccessListener successListener, final SPFailuredListener failuredListener){

		assert(successListener!=null);
		assert(failuredListener!=null);
//		Information/comment_list
		String url =  SPMobileHttptRequest.getRequestUrl("Quack", "quack_cusers");
	...
	public static void getComCheckUserWithCID(String cid, String uid, String status, final SPSuccessListener successListener, final SPFailuredListener failuredListener){
...
//							comUser = SPJsonUtil.fromJsonArrayToList(response.getJSONArray("data"), ComUser.class);
	 ...
	 * @param failuredListener
	 */
	public static void quitQuackWithCID(String type,String cid , final SPSuccessListener successListener, final SPFailuredListener failuredListener){
...
//		Information/comment_list
	...
	/**审核成员
	 * //	Quack/     cid（门派id）、uid（待审核成员uid）、status（审核状态  1 通过  -1 拒绝）
	 * Quack/quack_check
	 * @param cid
	 * @param uid
	 * @param status
	 ...
	 * @param failuredListener
	 */
	public static void checkQuackWithCID(String cid ,String uid,String status, final SPSuccessListener successListener, final SPFailuredListener failuredListener){
...
//	名称：Quack/del_user
//	参数：uid 需删除成员id、cid 门派id
	public static void deleteQuackUserWithID(String cid ,String uid, final SPSuccessListener successListener, final SPFailuredListener failuredListener){

		assert(successListener!=null);
		assert(failuredListener!=null);
//		Information/comment_list
		String url =  SPMobileHttptRequest.getRequestUrl("Quack", "del_user");
		RequestParams params = new RequestParams();
		params.put("cid" , cid);
		params.put("uid" , uid);
		SPMobileHttptRequest.post(url, params, new JsonHttpResponseHandler() {
