...
public class AddComArticleActivity extends BaseActivity implements View.OnClickListener {
    private ImageView imageView;    //图片
    private EditText  editTextTitle;  //标题
    private EditText  editTextContent;//内容
    private Button rithtBtn; //右边按钮
    private String cid ;//门派id

    Bitmap mBitmap;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.setCustomerTitle(true, true, "发表帖子", true);
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_add_com_article);
        super.init();
    }
        ...
        switch (v.getId()){
            ...
            case R.id.titlebar_menu_btn:
                ...
                if (SPStringUtils.isEditEmpty(editTextTitle)) {
                    editTextTitle.setError(Html.fromHtml("<font color='red'>" + getString(R.string.jh_article_name_null) + "</font>"));
                    return;
                }
                if (SPStringUtils.isEditEmpty(editTextContent)) {
                    editTextContent.setError(Html.fromHtml("<font color='red'>"+getString(R.string.jh_article_content_null)+"</font>"));
                    return;
                }
                if (mBitmap ==null) {
                    showToast("请选择图片");
                ...
                compuPlishArticle(mBitmap,cid,editTextTitle.getText().toString().trim(),editTextContent.getText().toString().trim());
...
//                        compuPlishArticle(mBitmap);
     ...
     * @param mBitmap
     */
    private void compuPlishArticle(Bitmap mBitmap,String cid,String title,String description) {
        ByteArrayOutputStream bytes = new ByteArrayOutputStream();
        ...
        fileList.add(header);
        CommunityRequest.CompuPlishArticle(cid,title,description,fileList, new SPSuccessListener() {
            @Override
            public void onRespone(String msg, Object response) {
                showToast(msg);
                setResult(MobileConstants.Result_Code_Refresh);
                AddComArticleActivity.this.finish();
            }
        }, new SPFailuredListener(AddComArticleActivity.this) {
            @Override
...
//            mSendComTxtv.setTextColor(MyColor.WHITE);
          ...
          String.valueOf(8 - editTextTitle.length());
            if(editTextTitle.length()>12){
                showToast("标题不能超过12个字");
